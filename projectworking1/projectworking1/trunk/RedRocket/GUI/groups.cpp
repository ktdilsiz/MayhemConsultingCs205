    ...
    ui(new Ui::Groups)
{
    ...
    for (unsigned int i = 1; i < session_manager->getGroups().size(); i++){
...
void Groups::on_pushButton_startchat_clicked()
{
    ...
    session_manager->setGroup(ui->lineEdit_enter_groupname->text().toStdString());
    grouppage = new GroupPage(session_manager);
    grouppage->show();
}
