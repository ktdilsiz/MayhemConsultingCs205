...
std::vector<std::vector<std::string> >* DBHandler::runQuery(std::string query) {
    ...
    *log_ << "DBHandler: Connection to local db opened";
    ...
    //log
    //*log_ << "DBHandler: Query evaluated with result code " + std::to_string(result_code) + " with error code: " + *error;
...
//MAY BE SAME AS THE RUNQUERY BUT DIDNT WANNA RISK CHANGING IT
std::vector<std::vector<std::string> >* DBHandler::runQueryMessages(std::string query) {
    //open database connection
    sqlite3_open("../../../db_tools/localDB.db", &db_connection_);
    //log
    *log_ << "DBHandler: Connection to local db opened";
