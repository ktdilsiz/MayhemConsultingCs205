...
std::vector<std::vector<std::string> > SessionManager::getMessagesDirect(string user1, string user2) {
    ...
    //std::string query = "SELECT time,content,recipient FROM " + user1 + "_messages WHERE recipient='" + user2 + "' INNER JOIN " + user2 + "_messages ON " + user2 + "_messages.recipient='" + user1 + "' ORDER BY time;";
    //run query
    dump = *(db->runQuery(query));
    //return the vector
    return dump;
...
std::vector<std::vector<std::string> > SessionManager::getMainFeed(){
    ...
    ss << "SELECT time AS posttime,content,sendername FROM " + groups[0];
    //loop through all groups to build query
    if(groups.size() > 1){
        for(int i=1; i<groups.size(); i++) {
            ss << " UNION ALL SELECT time AS posttime,content,sendername FROM " + groups[i];
        }
    }
    //sort by time, end query
    ss << " ORDER BY posttime;";
